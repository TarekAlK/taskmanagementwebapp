...
export default function Tasks({ setUpdateConfirmTaskFlag }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try { 
        ...
        setTodos(data.data.todoTasks)
        setDoings(data.data.doingTasks)
        setDones(data.data.doneTasks)
      } catch (error) {
        console.log(error)
      }
  }
  ...
  // async function handleTodo(todo, i) {
  //   const data = await axios.get(`http://localhost:5000/api/v1/tasks/${todo._id}`)
  ...
  async function deleteTask(e, tipe, i) {
    e.stopPropagation()
    await axios.delete(`http://localhost:5000/api/v1/tasks/${tipe._id}`)
    if(tipe.stats === 'todo') {
      setUpdateFlag(false)
