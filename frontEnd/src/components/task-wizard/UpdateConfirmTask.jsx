...
export default function UpdateConfirmTask({ setUpdateConfirmTaskFlag, setWizardPop }) {
  ...
  const checkboxChangeFlag = useRef(false)
  ...
  const setPrevType = baseSelectedOption === 'todo' ? setTodo : baseSelectedOption === 'doing' ? setDoing : setDone
    ...
    function handleCheckboxChange(e, i) {
      if(prevType[index].subtasks[i].completed === e.target.checked){
        checkboxChangeFlag.current = false
      } else{
        checkboxChangeFlag.current = true
      }
      const prev = [...prevType]
    ...
    async function onClick() {
      try {
        //const response = 
        await axios.patch(`http://localhost:5000/api/v1/tasks/${prevType[index]._id}/completed`, {
          "subtasks": prevType[index].subtasks,
          "stats": prevType[index].stats,
          "id": prevType[index]._id
        })
        ...
        <div className='flex flex-col gap-2 mb-4 shadow-md'>
            {
                //useMemo(() => type, [])[index].subtasks.map((subtask, i) => {
                  //(prevType[index].subtasks !== undefined) && prevType[index].subtasks.map((subtask, i) => {
                  (type[index].subtasks !== undefined) && type[index].subtasks.map((subtask, i) => {
                  return (
